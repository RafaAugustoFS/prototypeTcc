...
public class StudentController {
     ...
     *
     * @param loginStudent DTO contendo o código de identificação e a senha do estudante.
    ...
    public ResponseEntity<Map<String, String>> loginStudent(@RequestBody LoginStudent loginStudent) {
        try {
            ...
            return ResponseEntity.status(HttpStatus.UNAUTHORIZED).body(Map.of("error", "Falha no login: " + e.getMessage()));
        }
    }
    ...
    public ResponseEntity<?> criarEstudante(@RequestBody StudentClassDTO studentDTO) {
        try {
            ...
            return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(Map.of("error", "Erro ao criar estudante: " + e.getMessage()));
        }
    }
    ...
    public ResponseEntity<?> buscarTodosEstudantes() {
        try {
            ...
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(Map.of("error", "Erro ao buscar estudantes: " + e.getMessage()));
        }
    }
     ...
     * @return ResponseEntity com o estudante no formato DTO ou uma mensagem de erro.
     */
    ...
    public ResponseEntity<?> buscarEstudantePorId(@PathVariable Long id) {
        try {
            ...
            List<NoteDTO> notas = estudante.getNotas().stream()
                .map(nota -> new NoteDTO(nota.getId(), nota.getNota(), nota.getBimestre(), nota.getStatus(), nota.getDisciplineId().getNomeDisciplina()))
                .collect(Collectors.toList());

            ClassDTO turma = new ClassDTO(estudante.getClassSt().getNomeTurma(), estudante.getClassSt().getId());
            StudentDTO studentDTO = new StudentDTO(
                estudante.getNomeAluno(),
                estudante.getDataNascimentoAluno().toString(),
            ...
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(Map.of("error", "Erro ao buscar estudante: " + e.getMessage()));
        }
    }
    ...
    public ResponseEntity<?> atualizarEstudante(@PathVariable Long id, @RequestBody Student student) {
        try {
            ...
            return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(Map.of("error", "Erro ao atualizar estudante: " + e.getMessage()));
        }
    }
    ...
    public ResponseEntity<?> deletarEstudante(@PathVariable Long id) {
        try {
            ...
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(Map.of("error", "Erro ao deletar estudante: " + e.getMessage()));
        }
    }
    ...
    public ResponseEntity<?> uploadImage(@PathVariable Long id, @RequestBody Map<String, String> request) {
        try {
            ...
            return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(Map.of("error", "Erro ao fazer upload da imagem: " + e.getMessage()));
        }
    }
    ...
    public ResponseEntity<?> getImage(@PathVariable Long id, @RequestHeader("Authorization") String authHeader) {
        try {
            ...
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(Map.of("error", "Erro ao recuperar a imagem: " + e.getMessage()));
        }
    }
